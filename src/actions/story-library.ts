...
import {
  ...
  isUsableAudioTextStatus,
  type AudioCandidateStatus,
  type AudioReadinessChapterInput,
} from "@/lib/story-library/audio-readiness";
import {
  planEpisode,
  buildEpisodeScriptPreview,
  DEFAULT_EPISODE_PLANNER_OPTIONS,
  WORDS_PER_AUDIO_MINUTE as EPISODE_WORDS_PER_AUDIO_MINUTE,
  type EpisodePlannerChapterInput,
} from "@/lib/story-library/audio-episode-planner";
import { buildSampleScript, type SampleDurationMinutes } from "@/lib/story-library/audio-preview-sample-script";
import {
  generateEpisodePreviewSample,
  getLatestEpisodePreviewSample,
  writeBlockedSamplePreview,
} from "@/lib/story-library/audio-preview-sample-tts";
import { analyzeAudioContentQuality, TTS_BLOCKED_MESSAGE } from "@/lib/story-library/audio-content-quality";
import {
...
function toAudioReadinessChapterInput(chapter: {
  ...
  // Only present when the caller already loaded full chapter rows (story detail /
  // episode preview). When the raw word_count is 0 — e.g. a chapter whose original
  // crawl failed/was missing and was later recovered entirely via manual audio text —
  // fall back to counting the actual audio text so it isn't wrongly treated as empty.
  audioText?: string | null;
}): AudioReadinessChapterInput {
  const effectiveWordCount =
    chapter.wordCount === 0 && isUsableAudioTextStatus(chapter.audioTextStatus) && chapter.audioText
      ? countWords(chapter.audioText)
      : chapter.wordCount;
  return {
    ...
    recoveryStatus: chapter.recoveryStatus,
    recoveryMethod: chapter.recoveryMethod,
    recoveredFromSourceSite: chapter.recoveredFromSourceSite,
    recoveredFromUrl: chapter.recoveredFromUrl,
    audioTextStatus: chapter.audioTextStatus ?? null,
    audioTextSource: chapter.audioTextSource ?? null,
  };
}
...
export async function getStoryLibraryStoryAction(storyId: string) {
  ...
  const audioReadiness = computeAudioReadiness(
    chapters.map(toAudioReadinessChapterInput),
...
export type StoryLibraryAudioCandidateFilters = {
  status?: AudioCandidateStatus | "all";
  genre?: string;
  sourceSite?: string;
  minContiguousChapters?: number;
  minEstimatedMinutes?: number;
  hasRecoveredChapters?: boolean;
  hasMissingChapters?: boolean;
  page?: number;
  pageSize?: number;
};

export type StoryLibraryAudioCandidateRow = ReturnType<typeof computeAudioReadiness> & {
  id: string;
  title: string;
  sourceSite: string;
  author: string | null;
...
function matchesAudioCandidateFilters(row: StoryLibraryAudioCandidateRow, filters: StoryLibraryAudioCandidateFilters) {
  if (filters.status && filters.status !== "all" && row.status !== filters.status) return false;
  if (filters.genre && !row.genres.includes(filters.genre)) return false;
  if (filters.sourceSite && row.sourceSite !== filters.sourceSite) return false;
  if (filters.minContiguousChapters && row.contiguousDoneFromStart < filters.minContiguousChapters) return false;
  if (filters.minEstimatedMinutes && row.estimatedAudioMinutes < filters.minEstimatedMinutes) return false;
  if (filters.hasRecoveredChapters && row.recoveredChapters <= 0) return false;
  // Blocking/filtering uses the audio-aware unresolved count, not raw missing —
  // a raw-missing chapter with an approved manual audio_text must not count as blocking.
  if (filters.hasMissingChapters && row.unresolvedMissingForAudioCount <= 0) return false;
  return true;
}

// Default composite ordering: ready stories first, then by estimated audio minutes,
// contiguous chapters, unresolved-for-audio count, and most-recently-updated.
function compareAudioCandidateRows(a: StoryLibraryAudioCandidateRow, b: StoryLibraryAudioCandidateRow) {
  const aReady = a.readyForPreview ? 1 : 0;
  const bReady = b.readyForPreview ? 1 : 0;
  if (aReady !== bReady) return bReady - aReady;
  if (a.estimatedAudioMinutes !== b.estimatedAudioMinutes) return b.estimatedAudioMinutes - a.estimatedAudioMinutes;
  if (a.contiguousDoneFromStart !== b.contiguousDoneFromStart) return b.contiguousDoneFromStart - a.contiguousDoneFromStart;
  if (a.unresolvedMissingForAudioCount !== b.unresolvedMissingForAudioCount) {
    return a.unresolvedMissingForAudioCount - b.unresolvedMissingForAudioCount;
  }
  return b.updatedAt.getTime() - a.updatedAt.getTime();
}

export async function getStoryLibraryAudioCandidatesAction(filters: StoryLibraryAudioCandidateFilters = {}) {
  const [storyRows, chapterRows, filterOptions] = await Promise.all([
    db
      .select({
        id: storySources.id,
        sourceSite: storySources.sourceSite,
        title: storySources.title,
        author: storySources.author,
        genres: storySources.genres,
        crawlStatus: storySources.crawlStatus,
        chapterCount: storySources.chapterCount,
        updatedAt: storySources.updatedAt,
      })
      .from(storySources),
    db
      .select({
        storyId: storySourceChapters.storyId,
        chapterNumber: storySourceChapters.chapterNumber,
        crawlStatus: storySourceChapters.crawlStatus,
        wordCount: storySourceChapters.wordCount,
        lastError: storySourceChapters.lastError,
        recoveryStatus: storySourceChapters.recoveryStatus,
        recoveryMethod: storySourceChapters.recoveryMethod,
        recoveredFromSourceSite: storySourceChapters.recoveredFromSourceSite,
        recoveredFromUrl: storySourceChapters.recoveredFromUrl,
        audioTextStatus: storySourceChapters.audioTextStatus,
        audioTextSource: storySourceChapters.audioTextSource,
      })
      .from(storySourceChapters),
    getStoryLibraryFilterOptionsAction(),
  ]);

  const chaptersByStoryId = new Map<string, AudioReadinessChapterInput[]>();
  for (const chapter of chapterRows) {
    const list = chaptersByStoryId.get(chapter.storyId) ?? [];
    list.push(toAudioReadinessChapterInput(chapter));
    chaptersByStoryId.set(chapter.storyId, list);
  }

  const allRows: StoryLibraryAudioCandidateRow[] = storyRows.map((story) => {
    const chapters = chaptersByStoryId.get(story.id) ?? [];
    const readiness = computeAudioReadiness(chapters, { chapterCount: story.chapterCount });
    return {
      ...readiness,
      id: story.id,
      title: story.title,
      ...
      crawlStatus: story.crawlStatus,
      updatedAt: story.updatedAt,
    };
  });

  const summary = {
    readyForPreviewCount: allRows.filter((row) => row.status === "ready_for_preview").length,
    readyForOneHourEpisodeCount: allRows.filter((row) => row.readyForOneHourEpisode).length,
    needsMoreChaptersCount: allRows.filter((row) => row.status === "needs_more_chapters").length,
    blockedByMissingChaptersCount: allRows.filter((row) => row.status === "blocked_by_missing_chapters").length,
    hasFailedChaptersCount: allRows.filter((row) => row.status === "has_failed_chapters").length,
    noTextYetCount: allRows.filter((row) => row.status === "no_text_yet").length,
    needsReviewCount: allRows.filter((row) => row.status === "needs_review").length,
    recoveredChaptersTotal: allRows.reduce((sum, row) => sum + row.recoveredChapters, 0),
    estimatedAudioHoursAvailable: Number(
      (allRows.reduce((sum, row) => sum + row.estimatedAudioMinutes, 0) / 60).toFixed(1)
    ),
  };

  const filtered = allRows.filter((row) => matchesAudioCandidateFilters(row, filters));
  filtered.sort(compareAudioCandidateRows);
...
function toEpisodePlannerChapterInput(chapter: {
  id: string;
  chapterNumber: number;
  chapterTitle: string | null;
  chapterUrl: string;
  ...
  audioTextStatus: string;
  audioTextSource: string | null;
}): EpisodePlannerChapterInput {
  // Prefer the normalized/approved audio text layer (whether derived automatically
  // or manually imported) for TTS-bound script building; raw content_text is the
  // fallback (and is always what selection/word-count math uses via the wordCount
  // DB column — EXCEPT when that raw column is 0, e.g. a chapter whose original
  // crawl failed/was missing and was later recovered entirely via manual audio
  // text. In that case fall back to counting the actual audio text, otherwise the
  // chapter would have real usable text but still be skipped as "0 words").
  const usesAudioText = chapter.audioTextStatus === "normalized" || chapter.audioTextStatus === "approved";
  const effectiveWordCount =
    chapter.wordCount === 0 && usesAudioText && chapter.audioText ? countWords(chapter.audioText) : chapter.wordCount;
  return {
    id: chapter.id,
    chapterNumber: chapter.chapterNumber,
    chapterTitle: chapter.chapterTitle,
    chapterUrl: chapter.chapterUrl,
    contentText: usesAudioText && chapter.audioText ? chapter.audioText : chapter.contentText,
    crawlStatus: chapter.crawlStatus,
    wordCount: effectiveWordCount,
    lastError: chapter.lastError,
    recoveryStatus: chapter.recoveryStatus,
...
export type StoryLibraryEpisodePreviewOptions = {
  targetMinutes?: number;
  startChapterNumber?: number;
};

export async function getStoryLibraryEpisodePreviewAction(storyId: string, options: StoryLibraryEpisodePreviewOptions = {}) {
  const story = await db.query.storySources.findFirst({
    where: eq(storySources.id, storyId),
  });
  if (!story) return null;
  ...
  const readiness = computeAudioReadiness(chapters.map(toAudioReadinessChapterInput), { chapterCount: story.chapterCount });

  const targetMinutes = options.targetMinutes && options.targetMinutes > 0
    ? options.targetMinutes
    : DEFAULT_EPISODE_PLANNER_OPTIONS.targetMinutes;
  const startChapterNumber = options.startChapterNumber && options.startChapterNumber > 0
    ? options.startChapterNumber
    : DEFAULT_EPISODE_PLANNER_OPTIONS.startChapterNumber;

  // Derive min/max minutes and word band proportionally to the requested target, anchored
  // to the same ratios as the defaults (60 -> 55..65 minutes, 20k -> 18k..22k words).
  const targetWords = Math.round(targetMinutes * EPISODE_WORDS_PER_AUDIO_MINUTE);

  const plan = planEpisode(
    { id: story.id, title: story.title, sourceSite: story.sourceSite },
    chapters.map(toEpisodePlannerChapterInput),
    {
      targetMinutes,
      minMinutes: Math.max(1, targetMinutes - 5),
      maxMinutes: targetMinutes + 5,
      targetWords,
  ...
  const script = buildEpisodeScriptPreview(plan.selectedChapters);

  const contentQuality = analyzeAudioContentQuality({
    chapters: plan.selectedChapters.map((chapter) => ({
      chapterNumber: chapter.chapterNumber,
      text: chapter.contentText ?? "",
    })),
  });

  // Pre-TTS audio text review state for the currently selected chapter range.
  const selectedAudioTextAnalyses = plan.selectedChapters.map((chapter) => ({
    chapterNumber: chapter.chapterNumber,
    audioTextStatus: chapter.audioTextStatus ?? "raw",
    analysis: analyzeChapterAudioText({ text: chapter.contentText ?? "", chapterNumber: chapter.chapterNumber }),
  }));
  const audioTextSummary = {
    approvedCount: selectedAudioTextAnalyses.filter((c) => c.audioTextStatus === "approved").length,
    normalizedCount: selectedAudioTextAnalyses.filter((c) => c.audioTextStatus === "normalized").length,
    needsReviewCount: selectedAudioTextAnalyses.filter(
      (c) => c.audioTextStatus === "needs_review" || c.analysis.status === "needs_review"
    ).length,
    blockedCount: selectedAudioTextAnalyses.filter((c) => c.audioTextStatus === "blocked" || c.analysis.status === "block").length,
    rawCount: selectedAudioTextAnalyses.filter((c) => c.audioTextStatus === "raw").length,
    blockingIssueExamples: selectedAudioTextAnalyses
      .flatMap((c) => c.analysis.issues.filter((issue) => issue.severity === "block"))
      .slice(0, 10),
  };
...
export async function getStoryLibraryEpisodePreviewSampleAction(storyId: string) {
  return getLatestEpisodePreviewSample(storyId);
}

function parseSampleDurationMinutes(value: FormDataEntryValue | null): SampleDurationMinutes {
  return Number(value) === 3 ? 3 : 5;
}

export async function createStoryLibraryEpisodePreviewSampleAction(formData: FormData) {
  const storyId = String(formData.get("story_id") ?? "");
  if (!storyId) return;

  const targetMinutes = Number(formData.get("target_minutes") ?? 60) || DEFAULT_EPISODE_PLANNER_OPTIONS.targetMinutes;
  const startChapterNumber = Number(formData.get("start_chapter") ?? 1) || DEFAULT_EPISODE_PLANNER_OPTIONS.startChapterNumber;
  const sampleMinutes = parseSampleDurationMinutes(formData.get("sample_minutes"));

  const preview = await getStoryLibraryEpisodePreviewAction(storyId, { targetMinutes, startChapterNumber });
  if (!preview) return;

  const sample = buildSampleScript(
    { introText: preview.script.introText, bodyText: preview.script.bodyText, outroText: preview.script.outroText },
    sampleMinutes
  );

  // Pre-TTS Content Verification gate, per selected chapter — catches star/dot
  // obfuscation, placeholder/gate noise, and chapter markers even on chapters that
  // were never individually normalized/approved (e.g. still "raw" or incorrectly
  // marked "Đã kiểm tra" by a human without the audio text layer being checked).
  const blockingChapter = preview.plan.selectedChapters
    .map((chapter) => ({
      chapterNumber: chapter.chapterNumber,
      audioTextStatus: chapter.audioTextStatus ?? "raw",
      analysis: analyzeChapterAudioText({ text: chapter.contentText ?? "", chapterNumber: chapter.chapterNumber }),
    }))
    .find((entry) => entry.audioTextStatus === "blocked" || entry.analysis.status === "block");

  if (blockingChapter) {
    const blockerIssue = blockingChapter.analysis.issues.find((issue) => issue.severity === "block");
    const reason =
      blockingChapter.audioTextStatus === "blocked"
        ? "audio_text_status is blocked for this chapter"
        : blockerIssue
          ? `${blockerIssue.issueType} ("${blockerIssue.snippet}")`
          : "unresolved blocking issue";
    writeBlockedSamplePreview({
      storyId,
      startChapterNumber: preview.plan.startChapterNumber,
      endChapterNumber: preview.plan.endChapterNumber,
      sample,
      message: `${TTS_BLOCKED_MESSAGE} Chapter ${blockingChapter.chapterNumber}: ${reason}.`,
    });
    safeRevalidatePath(`/admin/story-library/audio-candidates/${storyId}/episode-preview`);
    return;
  }

  // No selected chapter is blocking — still run the safe mechanical normalizer over
  // the assembled sample body so dot-obfuscation/spacing artifacts in chapters that
  // were never individually normalized (still "raw" but warn/pass-level) are fixed
  // before this text is sent to TTS.
  const bodyNormalization = normalizeChapterAudioText({ text: sample.bodyText });
  const sampleForTts = { ...sample, bodyText: bodyNormalization.normalizedText };
  const usedNormalizedText = bodyNormalization.normalizedText !== sample.bodyText;

  await generateEpisodePreviewSample({
    storyId,
    startChapterNumber: preview.plan.startChapterNumber,
    endChapterNumber: preview.plan.endChapterNumber,
    sample: sampleForTts,
    contentQualityStatus: bodyNormalization.status,
    usedNormalizedText,
  });
