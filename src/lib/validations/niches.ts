...
export const nicheSchema = z.object({
  ...
  // Media config
  musicFolder: z.string().max(200).optional(), // e.g. "phat-phap", "truyen-audio"
  videoType: z.enum(["short", "long", "both"]).default("both"),
  ttsVoice: z.string().max(50).default("Ly"), // VieNeu-TTS voice ID
});
