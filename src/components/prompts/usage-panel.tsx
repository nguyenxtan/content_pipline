...
export function UsagePanel({ nicheId, stage, refreshKey }: UsagePanelProps) {
  ...
  if (!summary || summary.totalRuns === 0) {
    return (
      <div className="rounded-md border border-[hsl(var(--border))] p-4 space-y-1">
      ...
      <div className="space-y-1">
        ...
        <div className="space-y-1 max-h-48 overflow-y-auto">
          {runs.map((r) => (
            <div
              key={r.id}
              className="flex items-center justify-between gap-2 rounded px-2 py-1.5 hover:bg-[hsl(var(--accent))] transition-colors"
            >
              <div className="flex items-center gap-2 min-w-0">
                ...
                />
                <span className="text-xs font-mono text-[hsl(var(--muted-foreground))] truncate max-w-[90px]">
                  {r.model.split("/").pop()}
                </span>
              </div>
...
function StatCard({
  ...
  return (
    <div className="rounded-md bg-[hsl(var(--card))] border border-[hsl(var(--border))] px-3 py-2">
      <p className="text-xs text-[hsl(var(--muted-foreground))]">{label}</p>
      <p
        className={`text-sm font-semibold ${highlight ? "text-amber-400" : "text-[hsl(var(--foreground))]"}`}
      >
        {value}
      </p>
    </div>
