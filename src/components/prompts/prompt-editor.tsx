...
const CodeMirror = dynamic(() => import("@uiw/react-codemirror"), {
  ssr: false,
  loading: () => (
    <div className="h-[400px] rounded-md border border-[hsl(var(--input))] bg-[hsl(var(--card))] flex items-center justify-center text-sm text-[hsl(var(--muted-foreground))]">
      Đang tải editor...
    </div>
  ),
});
        ...
        <div className="flex items-center justify-between gap-2">
          <div className="relative">
            <button
              ...
              className="flex items-center gap-1.5 rounded-md border border-[hsl(var(--border))] px-3 py-1.5 text-xs text-[hsl(var(--muted-foreground))] hover:bg-[hsl(var(--accent))] hover:text-[hsl(var(--accent-foreground))] transition-colors"
            >
              Insert Variable
              <ChevronDown className="h-3 w-3" />
            </button>
            {showInsertMenu && (
              <div className="absolute top-full left-0 mt-1 z-10 min-w-[160px] rounded-md border border-[hsl(var(--border))] bg-[hsl(var(--popover))] shadow-lg py-1">
                {(STAGE_VARIABLES[stage] ?? []).map((v) => (
                  <button
                    key={v}
                    type="button"
                    onClick={() => handleInsertVariable(v)}
                    className="w-full text-left px-3 py-1.5 text-xs font-mono text-[hsl(var(--foreground))] hover:bg-[hsl(var(--accent))] transition-colors"
                  >
                    {`{{${v}}}`}
                  </button>
                ))}
        ...
        <div className="grid grid-cols-3 gap-4">
          ...
          <div className="space-y-1.5">
            ...
            <input
              ...
              className="w-full rounded-md border border-[hsl(var(--input))] bg-transparent px-3 py-2 text-sm text-[hsl(var(--foreground))] focus:outline-none focus:ring-2 focus:ring-[hsl(var(--ring))]"
            />
          </div>
        </div>
      ...
      <div className="flex-[2] min-w-0 space-y-5">
              ...
              {detectedVars.map((v) => (
                <div key={v} className="space-y-1">
                  ...
                  <input
                    ...
                    placeholder={`Nhập ${v}...`}
                    className="w-full rounded-md border border-[hsl(var(--input))] bg-transparent px-3 py-1.5 text-sm text-[hsl(var(--foreground))] placeholder:text-[hsl(var(--muted-foreground))] focus:outline-none focus:ring-2 focus:ring-[hsl(var(--ring))]"
                  />
                </div>
              ))}
            </div>
        ...
        {versions.length > 0 && (
          <div className="rounded-md border border-[hsl(var(--border))] p-4 space-y-3">
            ...
            {selectedVersion && (
              <pre className="text-xs text-[hsl(var(--muted-foreground))] bg-[hsl(var(--card))] rounded p-3 overflow-auto max-h-48 whitespace-pre-wrap break-words border border-[hsl(var(--border))]">
...
function OutputDisplay({ output }: { output: string }) {
  ...
  return (
    <pre className="text-xs text-[hsl(var(--foreground))] bg-[hsl(var(--card))] rounded p-3 overflow-auto max-h-64 whitespace-pre-wrap break-words border border-[hsl(var(--border))]">
      {pretty ?? output}
    </pre>
  );
}
